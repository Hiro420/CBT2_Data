...
local function CreateSceneData(schemePath)
			...
			for k, v in pairs(self.groups) do
				...
				if v ~= nil and v.suites ~= nil then
					...
					for i = 1, #(v.suites) do
						...
						if(suite.npcs~=nil) then
							for key,value in pairs(suite.npcs) do
	...
	for i=1 , #blocks do
		...
		for j=1 , #groups do
			...
			DoRequire(groupPath)
			--comment out unused data loading, only npcDic is in use, will re-evaluate the interface in the future
			--local groupInfo = { monsterDic = {}, npcDic = {}, gadgetDic = {}, regionDic = {}, triggerDic = {}, variables = {}, initConfig = {}, suites = {}}
			local groupInfo = { npcDic = {}}
