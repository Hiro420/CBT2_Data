...
--- 比较Euler角
-- @tparam Vector3 eulerA 欧拉角A
-- @tparam Vector3 eulerB 欧拉角B
-- @tparam float epsilon 容忍误差
M.CompareEuler = function(eulerA, eulerB, epsilon)
	if epsilon == nil then
		epsilon = 1
	end
	return csMath.CompareEuler(eulerA, eulerB, epsilon)
end

--- 返回向量夹角Euler角
-- @tparam Vector3 forwardA 欧拉角A
-- @tparam Vector3 forwardB 欧拉角B
M.ForwardAngle = function(forwardA, forwardB)
	return csMath.ForwardAngle(forwardA, forwardB)
end
