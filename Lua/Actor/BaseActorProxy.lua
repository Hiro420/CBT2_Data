...
function BaseActorProxy:CallDelay(delayTime, func, ...)
	...
	if self.uActor == nil then
		if NG_HSOD_DEBUG then
			print("call delay uActor nil " .. self.alias)
		end
		return cId
	end
...
function BaseActorProxy:SpawnMonster(configID, level, bornPos, bornDir, dropID, scale, alias, sceneID, questId, roomId)
	if sceneID == nil then
...
-- GadgetBornGadget = 5,
-- GadgetBornGround = 6,
function BaseActorProxy:SpawnItem(configID, bornPos, bornDir, alias, sceneID, bornType, questId, roomId)
	if sceneID == nil then
		sceneID = sceneData.DefaultSceneID
	end
	-- if sceneID ~= sceneData.currSceneID then
...
function BaseActorProxy:SpawnGadget(configID, bornPos, bornDir, alias, sceneID, dropId, questId, roomId)
	if sceneID == nil then
		sceneID = sceneData.DefaultSceneID
	end
	-- if sceneID ~= sceneData.currSceneID then
...
function BaseActorProxy:SpawnChest(configID, bornPos, bornDir, alias, sceneID, chestDropId, isShowCutscene, questId, roomId)
	if sceneID == nil then
		sceneID = sceneData.DefaultSceneID
	end
	if alias == nil then
...
function BaseActorProxy:PlayCutsceneIndex(index, OnFinish, OnStart, OnDestroy, syncLoad)
	actorUtils.ActorDebugError("The RenWuCeHua can only call PlayCutsceneIndex from questActor")
end
...
-- @tparam function OnStart 开始后的回调函数，这里是真正开始，定义需要包含一个actor（self）的参数
-- @tparam function OnDestroy 断线重连等销毁时候回调函数，定义需要包含一个actor（self）的参数
-- @tparam uint mainQuestId 关联任务id
function BaseActorProxy:PlayCutscene(config, OnFinish, OnStart, OnDestroy, mainQuestId)
	actorUtils.ActorDebugError("The RenWuCeHua can only call PlayCutsceneIndex from questActor")
end
...
-- @tparam function callback 倒计时结束回调
function BaseActorProxy:CountDownUIStart(duration, warningLimit, readyDuration,callback)
	if readyDuration == nil then
		readyDuration = 0
	end
	self.uActor:CountDownUIStart(duration, warningLimit, readyDuration,callback)
end
...
-- @tparam string textMapId 任务文字在TextMap中的Id
function BaseActorProxy:ShowBlackScreen(durationToBlack, durationKeepBlack, durationFromBlack, callback, finishCallback, task, textMapId)
	self.uActor:ShowBlackScreen(durationToBlack, durationKeepBlack, durationFromBlack, callback, finishCallback, task, textMapId)
end

--- 震屏方法 ShakeType为CustomVector
-- @tparam Vector3 shakeDir 方向
-- @tparam float shakeRange 中心振幅
-- @tparam float shakeTime 震时
-- @tparam float shakeDinstance 最大作用范围
function BaseActorProxy:TryAddShake(shakeDir,shakeRange,shakeTime,shakeDinstance)
	self.uActor:TryAddShake(shakeDir,shakeRange,shakeTime,shakeDinstance)
end

--- 显示教程界面的方法
-- @tparam string tutorialImgName 教程ID
-- @tparam Action callback 回调
function BaseActorProxy:ShowTutorialDialog(tutorialId, callback)
	self.uActor:ShowTutorialDialog(tutorialId, callback)
end

--- 显示任务图片的方法
-- @tparam string tutorialId 教程ID
-- @tparam Action callback 回调
function BaseActorProxy:ShowQuestPictureDialog(tutorialId, callback)
	self.uActor:ShowQuestPictureDialog(tutorialId, callback)
end

--- 显示阅读界面的方法
-- @tparam uint docId 阅读物ID
function BaseActorProxy:ShowReadingDialog(docId)
	if docId ~= nil then
		self.uActor:ShowReadingDialog(docId)
	end
end

--- 取消当前任务追踪
function BaseActorProxy:CancelCurrentNavigation()
	self.uActor:CancelCurrentNavigation()
end

--- 根据提供的MainId追踪任务
-- @tparam uint mainId 父Id
-- @tparam bool isQuest 是否为Quest类型
function BaseActorProxy:SetNavigationByMainId(mainId, isQuest)
	self.uActor:SetNavigationByMainId(mainId, isQuest)
end
...
-- @tparam function OnStart Timeline开始时候调用
function BaseActorProxy:PlayFreeCutscene(cfg, owner, OnFinish, OnStart)
	self.uActor:PlayFreeCutscene(cfg, owner, OnFinish, OnStart)
end
...
-- @tparam float fadeIn 淡入时间
-- @tparam float fadeOut 淡出时间
function BaseActorProxy:TryPlayVideo(videoName, subtitleId, canSkip, finishCallBack, bgColor,fadeIn,fadeOut)
	...
	self.uActor:TryPlayVideo(videoName, subtitleId, canSkip, finishCallBack, bgColor,fadeIn,fadeOut)
end
...
--- 改变Entity的材质属性
-- @tparam uint entityRuntimeId 实体ID
-- @tparam int propNameIndex 属性名称(1:EmissionIntensity 2:EmissionStrengthName)
-- @tparam float value 属性值
function BaseActorProxy:SetEntityMaterialPropValue(entityRuntimeId, propNameIndex, value)
	if entityRuntimeId == nil or propNameIndex == nil or value == nil then
		print("SetEntityMaterialPropValue Error!")
		return
	end
	self.uActor:SetEntityMaterialPropValue(entityRuntimeId, propNameIndex, value)
end

--- 播放NPC气泡表情
function BaseActorProxy:PlayEmojiBubble(patternName)
	self.uActor:PlayEmojiBubble(patternName)
end
...
function BaseActorProxy:RequestInteraction(alias)
	actorUtils.ActorDebugError("The RenWuCeHua can only call request interaction from questActor")
end
...
function BaseActorProxy:TransmitPlayer(sceneId, pos, rot, tcallback, tfinishcallback, task)
	return self.uActor:TryTransmitPlayer(sceneId, pos, rot, {}, 0, tcallback, tfinishcallback, task)
end
...
-- @tparam Vector3 targetRot 传送后的朝向
-- @tparam string[] textMapId 任务文字在TextMap中的Id
-- @tparam float textShowTime 单条任务文字显示时间
-- @tparam function tcallback 传送完成后的回调
-- @tparam function tfinishcallback 传送到黑屏时候的回调
-- @tparam CurtainTask task 传送黑屏时执行的任务，需要等task结束黑屏才能亮起
-- @return bool 返回该次传送是否成功
function BaseActorProxy:TransmitPlayerWithText(sceneId, pos, rot, textMapId, textShowTime, tcallback, tfinishcallback, task)
	return self.uActor:TryTransmitPlayer(sceneId, pos, rot, textMapId, textShowTime, tcallback, tfinishcallback, task)
end
...
function BaseActorProxy:SayWarningNarrator(dialogID)
	if self.mainQuestID ~= nil then
		self.uActor:SayWarningNarrator(dialogID, self.mainQuestID)
	else
...
--- Actor Trigger自己的Abitlity
function BaseActorProxy:TriggerAbility(abilityName)
	self.uActor:TriggerAbility(abilityName)
end
...
--- LUA显示临时文字提示，最终外放请用带textmap的：BaseActorProxy:ShowTextMapWithParam
-- @content string 用来在屏幕中间显示临时的文字
-- 注：如果param不为空, 要保证TextMap里有对应的占位符{0},{1}……
function BaseActorProxy:ShowMessage(content)
	actorUtils.ShowMessage(content)
end

--- LUA显示TextMap接口
-- @tparam string textMapId 在TextMap表中的ID
-- @tparam string[] param 用来拼接字符串的参数
-- 注：如果param不为空, 要保证TextMap里有对应的占位符{0},{1}……
-- 用例：self:ShowTextMapWithParam(TEXT_MAP_TEST_ID,{"test"})
function BaseActorProxy:ShowTextMapWithParam(textMapId, param)
	actorUtils.ShowTextMapWithParam(textMapId, param)
end

--- 返回当前玩家摄像机的位置
function BaseActorProxy:GetCameraPos()
	return actorUtils.GetCameraPos()
end

--- 返回当前玩家摄像机的欧拉角
function BaseActorProxy:GetCameraEuler()
	return actorUtils.GetCameraEuler()
end
...
-- @tparam uint mainQuestId
function BaseActorProxy:NarratorTask(dialogList, pauseLen, resumeLen, OnFinish, tag, mainQuestId)
...
function BaseActorProxy:NarratorTaskByData(data, OnFinish)
	self:NarratorTask(data.dialogList, data.pauseLen, data.resumeLen, OnFinish, data.tag, data.mainQuestID)
end
...
function BaseActorProxy:WarningNarratorTask(index, dialogList, OnCheckSpeak, OnFinish, tag, mainQuestId)
	if #dialogList < 0 then return nil end
...
--- 技能回调
-- @section ability
function BaseActorProxy:InvokeOnAbility(param)
end

--- 播放Dialog表中对应的语音
-- @dialogId uint 对话ID
function BaseActorProxy:SayDialogAudio(dialogId)
	if(self.uActor ~= nil) then
		self.uActor:SayDialogAudio(dialogId)
	end
end

--- 开启潜入模式
-- @sensingRange float 玩家的感知半径
...
--- 开启野外潜入模式
-- @sensingRange float 玩家的感知半径
function BaseActorProxy:EnterWildSneakMode(sensingRange)
	self.uActor:EnterWildSneakMode(sensingRange)
end

--- 关闭野外潜入模式
function BaseActorProxy:ExitWildSneakMode()
	self.uActor:ExitWildSneakMode()
end
...
function BaseActorProxy:ExitBomberMode()
	self.uActor:BackToNormalMode()
end

--- 开启FindCat模式
function BaseActorProxy:EnterFindCatMode()
	self.uActor:EnterFindCatMode()
end

--- 关闭FindCat模式
function BaseActorProxy:ExitFindCatMode()
	self.uActor:BackToNormalMode()
end
...
--- 注册元素视野开启回调
-- @tparam function func 一个没参数的函数
function BaseActorProxy:CallOnElemViewOpen(func)
	actorUtils.CallOnElemViewOpen(func)
end
	
--- 注册元素视野关闭回调
-- @tparam function func 一个没参数的函数
function BaseActorProxy:CallOnElemViewClose(func)
	actorUtils.CallOnElemViewClose(func)
end

--- 注册房间门被打开
-- @tparam function func 一个没参数的函数
function BaseActorProxy:CallOnDoorOpen(func)
	actorUtils.CallOnDoorOpen(func)
end

--- 取消注册房间门被打开
-- @tparam function func 一个没参数的函数
function BaseActorProxy:UncallOnDoorOpen()
	actorUtils.UncallOnDoorOpen()
end


--- 设置飞行状态参数
-- @tparam float flyBackSpeedMax
-- @tparam float flyHorizontalSpeedMax
-- @tparam float flyRotationAngularVelocity
-- @tparam float flyDownSpeed
-- @tparam float flyForwardSpeedMax
function BaseActorProxy:SetFlyStateParams(flyBackSpeedMax, flyHorizontalSpeedMax, flyRotationAngularVelocity, flyDownSpeed, flyForwardSpeedMax)
	if flyBackSpeedMax == nil then
		flyBackSpeedMax = -1
	end
	if flyHorizontalSpeedMax == nil then
		flyHorizontalSpeedMax = -1
	end
	if flyRotationAngularVelocity == nil then
		flyRotationAngularVelocity = -1
	end
	if flyDownSpeed == nil then
		flyDownSpeed = 1
	end
	if flyForwardSpeedMax == nil then
		flyForwardSpeedMax = -1
	end
	self.uActor:SetFlyStateParams(flyBackSpeedMax, flyHorizontalSpeedMax, flyRotationAngularVelocity, flyDownSpeed, flyForwardSpeedMax)
end

--- 重置飞行状态参数
function BaseActorProxy:ResetFlyStateParams()
	self.uActor:ResetFlyStateParams()
end
...
function BaseActorProxy:GetChallengeBestValue(challengeIndex)
	return self.uActor:GetChallengeBestValue(challengeIndex)
end

function BaseActorProxy:GetChallengeRecordCurrValue(challengeIndex)
	return self.uActor:GetChallengeRecordCurrValue(challengeIndex)
end

function BaseActorProxy:BattleUIStart(total, teammateID, teammateScore, opponentID, opponentScore)
	self.uActor:BattleUIStart(total, teammateID, teammateScore, opponentID, opponentScore)
end

function BaseActorProxy:BattleUIChange(teammateScore, teammateAdd, opponentScore, opponentAdd)
	self.uActor:BattleUIChange(teammateScore, teammateAdd, opponentScore, opponentAdd)
end

function BaseActorProxy:BattleUITerminate()
	self.uActor:BattleUITerminate()
end

function BaseActorProxy:IsTalkDialogActive()
	return self.uActor:IsTalkDialogActive()
end

function BaseActorProxy:IsGivenDialogActive()
	return self.uActor:IsGivenDialogActive()
end
