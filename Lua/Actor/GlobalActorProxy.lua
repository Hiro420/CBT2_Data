...
-- @tparam int avatarCfgId 角色配置id，如果不填就对当前角色
function GlobalActorProxy:SpawnAttach(attachName, avatarCfgId)
	if avatarCfgId == nil then
		avatarCfgId = 0
	end
	uActor:SpawnAttach(attachName, avatarCfgId)
end

--- 给角色去掉附件
-- @tparam string attachName 附件名称，在总配置里预先配好
-- @tparam int avatarCfgId 角色配置id，如果不填就对当前角色
function GlobalActorProxy:UnSpawnAttach(attachName, avatarCfgId)
	if avatarCfgId == nil then
		avatarCfgId = 0
	end
	uActor:UnSpawnAttach(attachName, avatarCfgId)
end
...
function GlobalActorProxy:DungeonFogEffects(enable, cameraFogEffectName, playerFogEffectName)
	self.uActor:DungeonFogEffects(enable, cameraFogEffectName, playerFogEffectName)
end
