...
local function EndGame()
	...
	if quest ~= nil then
		quest:FinishQuestID(false, 41602)
    end
    q416Cfg.NextNum = 1
end
...
local function CreateNext()
    if q416Cfg.NextNum < q416Cfg.CoinNum +1 then
        ...
        --     local mainQuest = actorMgr:GetActor(q416Cfg.ActorAlias)
        --     if mainQuest ~= nil then
        --         mainQuest:SpawnItemByIdWithPos(41601, 104011, 1, q416Cfg.CoinPos[i].pos, q416Cfg.CoinPos[i].rot, "BigApple" .. tostring(i))
        --     end
        -- end
        local quest = actorMgr:GetActor(q416Cfg.ActorAlias)
        if quest ~= nil then
            quest:FinishQuestID(false, 41603)
        end
...
function RunCoinStart:TriggerIn()
    ...
    if q416Cfg.NextNum < q416Cfg.CoinNum then
        self:PlayEffect("Eff_FlyRace_Marker_Active_AS", effctPos)
    else
        self:PlayEffect("Eff_FlyRace_Goal_Marker_AS", effctPos)
