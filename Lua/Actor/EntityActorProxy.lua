...
function EntityActorProxy:WalkTo(pos)
	if pos == nil then
		actorUtils.ActorDebugError(self.alias .. "lua position not invalid")
		return
	end
...
--- EntityActor对话结束的回调
function EntityActorProxy:InvokeOnFinalTalkFinish()
end
--- EntityActor转向一个方向
...
function EntityActorProxy:IsTalking()
	return self.uActor:IsTalking()
end
...
--- EntityActor隐藏
function EntityActorProxy:IsHided(value)
	return self.uActor:IsHided(value)
end
...
--- EntityActor找椅子坐下
function EntityActorProxy:SitOnChair(freeStyleType)
	self.uActor:SitOnChair(freeStyleType)
end
--- EntityActor从椅子上站起
...
--- Entity添加附件
-- @tparam string attachName 附件名称，在总配置里预先配好
function EntityActorProxy:SpawnAttach(attachName)
	self.uActor:SpawnAttach(attachName)
end

--- Entity删除附件
-- @tparam string attachName 附件名称，在总配置里预先配好
function EntityActorProxy:UnSpawnAttach(attachName)
	self.uActor:UnspawnAttach(attachName)
end
...
--- 设置优化范围，出了范围会被反激活
-- @tparam float dist 距离
function EntityActorProxy:SetLocalEntityDist(dist)
	self.uActor:SetLocalEntityDist(dist)
end

function EntityActorProxy:OnScriptEventTrigger(evtType)
	self.uActor:OnScriptEventTrigger(evtType)
end
