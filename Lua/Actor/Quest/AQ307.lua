...
function Quest307:OnSubStart30702(quest)
    ...
    function(actor, task)
        ...
        actorUtils.ClearCacheCmdExternal(actor.alias, ActorCommandType.MOVE)
...
function Quest307:OnSubStart30713(quest)
    self:ActionSafeCall(
        function (self)
            ...
            self:PlayCutsceneIndex(30701, self.On30713CutsceneFinish)
        end
    )
end
...
function Quest307:OnSubStart30715(quest)
    ...
    local task = self:CreateQuestNpcCreateTask({gaiaData.Gaia})
...
function Quest307:InvokeOnInteraction(param)
    if param == 1 then
