...
function Quest308:OnSubFinish30801(quest)
    self:ActionSafeCall(
        function ()
            ...
            function(actor, task)
                --region fix cache temply
                actorUtils.ClearCacheCmdExternal(actor.alias, ActorCommandType.MOVE_ROUTE)
...
function Quest308:InvokeOnInteraction(param)
    if param == 1 then
        print("Talk30811:destory paimon")
        self:ActionSafeCall(
            function(self)
                --local paimon = actorMgr:GetActor("Paimon308")
                local paimon = self:GetQuestNpcActor("Paimon")
