...
function Quest461:DestroyNpc()
    ...
    local npc = quest:GetQuestNpcActor(KaeyaNPCBossData.NpcBoss)
    if npc~=nil then
        npc:Destroy(false)
    end
end
...
function Quest461:MoveKaeya()
    local quest = actorMgr:GetActor(q461Cfg.ActorAlias)
    local kaeya = quest:GetQuestNpcActor(KaeyaData.Kaeya)
    if kaeya~=nil then
        kaeya:Destroy(false)
    end
    kaeya:Destroy(false)
    ...
    quest:CreateQuestNpcById(46102, KaeyaData.KaeyaID)
    --actorMgr:CreateActorWithPos(KaeyaData.Kaeya, KaeyaData.KaeyaScript, KaeyaData.KaeyaID, 0, KaeyaData.OutPos, KaeyaData.OutDir, true, false,3)
end
...
function Quest461:NPCTask()
    local npc = self:GetQuestNpcActor(KaeyaNPCBossData.NpcBoss)
    ...
    function(actor, task)
        ...
        --actor:ClearPriorityInter(InterTimingType.BTN)
        actor:EnableInteraction(true)
        actor:PerformDither(false, 2,actor.HideSelf)
        --actor:SetVisible(false)
        --actorMgr:CreateActorWithPos(KaeyaData.Kaeya, KaeyaData.KaeyaScript, KaeyaData.KaeyaID, 0, KaeyaData.InPos, KaeyaData.InDir, true, false,1004)
        --actor:Destroy(false)
        --actor:Standby()
...
function Quest461:OnSubStart46101(quest)
    ...
    --actorMgr:CreateActorWithPos(KaeyaData.Kaeya, KaeyaData.KaeyaScript, KaeyaData.KaeyaID, 0, KaeyaData.InPos, KaeyaData.InDir, true, false,1004)
    self:CreateQuestNpc(quest, KaeyaData.KaeyaID)
end
...
function Quest461:OnSubStart46102(quest)
    print("46102 Start : ")
    self:NotifyTo("Npc1606", DailyNpcManager.NpcEventType.HIDESELF,true)
    --actorMgr:CreateActorWithPos(KaeyaData.Kaeya, KaeyaData.KaeyaScript, KaeyaData.KaeyaID, 0, KaeyaData.OutPos, KaeyaData.OutDir, true, false,3)
    --[[self:ActionSafeCall(
        function(self)
            actorMgr:CreateActorWithPos(KaeyaData.Kaeya, KaeyaData.KaeyaScript, KaeyaData.KaeyaID, 0, KaeyaData.OutPos, KaeyaData.OutDir, true, false,3)
...
function Quest461:StartTalk()
    print("46302 Start : ")
    --actorMgr:CreateActorWithPos(KaeyaData.Kaeya, KaeyaData.KaeyaScript, KaeyaData.KaeyaID, 0, KaeyaData.OutPos, KaeyaData.OutDir, true, false,3)
    self:CreateQuestNpcById(46109, KaeyaData.KaeyaID)
    self:ActionSafeCall(
        function(self)
...
function Quest461:OnSubStart46103(quest)
    print("46103 Start : ")
    count=0
    self:NotifyTo("Npc1409", DailyNpcManager.NpcEventType.BANDAILY, true)
    --self:NotifyTo("Npc1411", DailyNpcManager.NpcEventType.BANDAILY, true)
...
function Quest461:CountTalk()
    count=count+1
    if count ==3 then
        local quest = self:GetQuestNpcActor(q461Cfg.ActorAlias)
        if quest ~= nil then
...
function Quest461:OnSubFinish46107(quest)
    print("46107 Finish : ")
    self:NotifyTo("Npc1409", DailyNpcManager.NpcEventType.STARTDAILY, true)
    --self:NotifyTo("Npc1411", DailyNpcManager.NpcEventType.STARTDAILY, true)
    self:NotifyTo("Npc1413", DailyNpcManager.NpcEventType.STARTDAILY, true)
    self:PaimonVanish()
    self:KaeyaTask3()
...
function Quest461:OnSubStart46107(quest)
    print("46107 Start : ")
    self:NotifyTo("Npc1606", DailyNpcManager.NpcEventType.STARTDAILY,true)
