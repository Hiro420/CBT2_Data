...
function Quest462:OnSubStart46201(quest)
    ...
    self:NotifyTo("Npc1525", DailyNpcManager.NpcEventType.BANDAILY, true)
end
...
function Quest462:OnSubStart46202(quest)
    ...
    --actorMgr:CreateActorWithPos(NPCData.Npc, NPCData.NPCScript, NPCData.NpcID, 0, NPCData.NPCBornPos, NPCData.NPCBornDir, true, false,3)
    self:CreateQuestNpc(quest, NPCData.NpcID)
    local KaeyaNPC = self:GetQuestNpcActor(NPCData.Npc)
...
function Quest462:OnSubStart46203(quest)
    ...
    self:NotifyTo("Npc1525", DailyNpcManager.NpcEventType.BANDAILY, true)
    local KaeyaNPC = self:GetQuestNpcActor(NPCData.Npc)
    KaeyaNPC:DoFreeStateTrigger()
    KaeyaNPC:Standby(true)
    KaeyaNPC:Standby()
    KaeyaNPC:EnableInteraction(true)
...
function Quest462:StartTalk46204()
    ...
    self:CreateQuestNpcById(46204, PaimonData.ID)
    self:CallDelay(0.2,
        function(self)
...
function Quest462:InvokeOnInteraction(param)
    ...
    if param == 0 then
        print("Invoke 0")
        self:ShowQuestPictureDialog(180)
    elseif param == 1 then
        self:ShowQuestPictureDialog(181)
    elseif param == 2 then
        self:ShowQuestPictureDialog(182)
    end    
end
...
function Quest462:OnSubFinish46206(quest)
    ...
    self:DestroyQuestNpcActor(KaeyaData.Kaeya, 1004)
end
