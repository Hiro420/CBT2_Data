...
function Quest371:OnDataLoaded()
    q371Cfg = self.clientData
    subIDs = q371Cfg.SubIDs
    paimonData = q371Cfg.PaimonData
    wendyData = q371Cfg.WendyData
end
...
function Quest371:OnSubStart37105(quest)
    ...
    self:SpawnGadget(quest , 70300037,1 )
    self:SpawnGadget(quest , 70300037,2 )
...
function Quest371:OnSubStart37108(quest)
    ...
    self:SpawnGadget(quest , 70300037,1 )
    self:SpawnGadget(quest , 70300037,2 )
    self:SpawnGadget(quest , 70300037,3 )
...
function Quest371:OnSubFinish37101(quest)
    ...
    local qin = self:GetQuestNpcActor("Qin")
    if qin ~= nil then
        qin:Destroy(false)
    end  

    local ambor = self:GetQuestNpcActor("Ambor")
    if ambor ~= nil then
        ambor:Destroy(false)
    end  

    local lisa = self:GetQuestNpcActor("Lisa")
    if lisa ~= nil then
        lisa:Destroy(false)
    end  
    
    local gaia = self:GetQuestNpcActor("Gaia")
    if gaia ~= nil then
        gaia:Destroy(false)
    end  
end
...
function Quest371:OnSubFinish37102(quest)
    ...
    self:ActionSafeCall(
        function(self)
            local wendy = self:GetQuestNpcActor("WendyNew")
            if wendy ~= nil then
                wendy:Destroy(false)
            end
        end
...
function Quest371:InvokeOnInteraction(param)
    if param == 11 then
        local paimon = self:GetQuestNpcActor(paimonData.Paimon)
        if paimon ~= nil then
            print("Now Spawn Emoji")
            -- paimon:PlayEmojiBubble("EmojiBubble_Emoji_Star")
            -- paimon:PlayAudio("Play_quest_lineDiluc_emotion_shine")
        end
    end
end
