...
function Quest390:OnDataLoaded()
    q390Cfg = self.clientData
    wendyData = q390Cfg.WendyData
    dilucData = q390Cfg.DilucData
    qinData = q390Cfg.QinData
    paimonData = q390Cfg.PaimonData
    organData = q390Cfg.OrganData
end
...
function Quest390:SealOpenCutscene()
    -- 获取任务状态 未完成 = 2, 已完成 = 3
    local firstorganstate = self:GetSubQuestState(39005)
    local secondorganstate = self:GetSubQuestState(39006)
    local thirdorganstate = self:GetSubQuestState(39007)
    print(firstorganstate.."|"..secondorganstate.."|"..thirdorganstate)
...
function Quest390:OnSubFinish39003(quest)
    print("39003 Finish:Camera Look")
    globalActor:EnablePlayerInput(false)
    self:EnterSceneLookCamera(sceneData:GetDummyPoint(3,"Q390Target2").pos, 0, 2, true)
    self:CallDelay(2,
        function (self)
            self:EnterSceneLookCamera(sceneData:GetDummyPoint(3,"Q394Target1").pos, 0, 3, true)
        end
    )
    self:CallDelay(5,
        function (self)
...
function Quest390:OnSubFinish39005(quest)
    print("39005 Finish:...")
    self:CallDelay(3, self.SealOpenCutscene)
end
...
function Quest390:OnSubFinish39008(quest)
    ...
    local quest = actorMgr:GetActor(q390Cfg.ActorAlias)
    if quest ~= nil then
        quest:FinishQuestID(false, 39009)
    end
end
