...
function Quest396:OnDataLoaded()
    q396Cfg = self.clientData
    wendyData = q396Cfg.WendyData
    bbrData = q396Cfg.BarbaraData
    qinData = q396Cfg.QinData
end
...
function Quest396:OnSubStart39602(quest)
    ...
    self:CreateQuestNpc(quest, bbrData.BarbaraID)
...
function Quest396:OnSubFinish39602(quest)
    ...
    self:ActionSafeCall(
        function(self)
            local bbr = self:GetQuestNpcActor(bbrData.Barbara)
            if bbr ~= nil then
                bbr:ClearFollowTask()
                bbr:Destroy(false)
            end  
