...
function Quest470:OnDataLoaded()
    q470Cfg = self.clientData
    saraData = q470Cfg.SaraData
    paimonData = q470Cfg.PaimonData
end
...
function Quest470:OnSubStart47002(quest)
    ...
    self:CallDelay(2, 
        function (self)
            ...
            self:CreateQuestNpcWithPos(quest, paimonData.PaimonID, 0, paimonPos, paimonRot, true, false)
            self:RequestInteraction(paimonData.Paimon)
        end
    )
end
...
function Quest470:OnSubStart47004(quest)
    ...
    self:ActionSafeCall(
        function (self)
            local paimonPos = actorUtils.GetAvatarPos()
            paimonPos.z = paimonPos.z + 1.2
            paimonPos.x = paimonPos.x + 0.8
            local paimonRot = {x=0,y=0,z=-1}

            self:CreateQuestNpcWithPos(quest, paimonData.PaimonID, 1, paimonPos, paimonRot)
            self:RequestInteraction(paimonData.Paimon)
        end
    )
end
