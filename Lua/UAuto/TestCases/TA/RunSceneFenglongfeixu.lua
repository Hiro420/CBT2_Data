...
local UAutoLuaTestCase = class("RunSceneFenglongfeixu",baseClass)
local self = UAutoLuaTestCase
self.TestFixture = "RunSceneFenglongfeixu"
self.Category = "TA测试" --自行修改分类
self.Description = "TA_遍历：风龙废墟" --自行修改用例描述

UAutoLuaTestCase.Step_1 = {
	Order = 1, --执行顺序
	Type = CaseType.MonoTest,
	Description = "遍历风龙废墟", --步骤描述
...
yield_return(self.base:Jump())
        ...
        -- 开启性能采集
        local gm_startStatUnit = string.format("StartStatUnit Test@Server-Case@FixedPoints@%s", 'TARunSceneFenglongfeixuDaily')
