...
local upLevelStep=3
function self:UpLevelCheck(avatarName,proudId)
	local proudId = proudId or 0
	...
	for promoteLevel=0,4 do
		...
		for level=startLevel,endLevel,upLevelStep do
			...
			self.base:CheckAvatarMaxHPByLevel(promoteLevel,level,maxHP,0,proudId)
			self.base:CheckAvatarAttackByLevel(promoteLevel, level, attackBase, 0, proudId)
...
self.QinLevelUpCheck = {
	...
	Action = UAutoCoroutine(function()
		yield_return(self:UpLevelCheck("Qin",31101))
	end)
}
...
self.DilucLevelUpCheck = {
	...
	Action = UAutoCoroutine(function()
		yield_return(self:UpLevelCheck("Diluc"),161101)
	end)
}
